 ...
 * @version    $Id: DeleteController.php 4 2009-6-1 Jaimie $
 */
class Picture_DeleteController extends Zend_Controller_Action
{
    ...
    public function artistImageAction()
    {
        ...
        if(!self::$acl->isAllowed($accountType, null, 'delete') ? "1" : "0") {
            $this->_redirect('error/access-denied/from/picture:delete:artist-image');
        }       
        ...
        $breadcrumbs[] = array('title' => $this->view->translate('Menu_Delete_Pictures'), 'url' => $this->view->baseUrl);

        $this->view->placeholder('breadcrumbs')->exchangeArray( Helper_Breadcrumb::process($breadcrumbs) );

        // get form
        $form = self::$artistForm->artistImageDelete($imageId);
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!$form->isValid($_POST)) {
                ...
                self::$artistDb->removeArtistImage($imageId);

                $this->_redirect('picture/delete/complete/artist/'.urlencode($artist['artist']));
            }
        } else {
            $this->view->form = $form;
        }
    ...
    public function completeAction()
    {
        ...
        $name = (string)$this->getRequest()->getParam('artist');
        $artist = self::$artistDb->getArtistByName($name);
        $this->view->artist = $artist;
