...
class Search_IndexController extends Zend_Controller_Action
{
    ...
    {       
        ...
        // if module is active
        if ( !$registry->get('moduleConfig')->module->search->enabled ) {
            $this->_redirect('/error/module-not-enabled/module/search');
        }
        ...
        self::$config      = $registry->get('moduleConfig')->module->search;
    }
    ...
    {  
        ...
        {
            case 'Artist':
                $this->_redirect('artist/'.urlencode($keyword));
            break;

            case 'Album':
                $this->_redirect('album/'.urlencode($keyword));
            break;

            case 'Genre':
                $this->_redirect('genre/'.urlencode($keyword));
            break;


            default:
                throw new exception('Missing required param type');
            break;
        }
    }
}
