 ...
 */
class Picture_AddController extends Zend_Controller_Action
{
    ...
    public function artistImageAction()
    {
        ...
        $artistName = $this->getRequest()->getParam('artist');
        ...
        $breadcrumbs[] = array('title' => $this->view->translate('Menu_Add_Pictures'), 'url' => $this->view->baseUrl);

        $this->view->placeholder('breadcrumbs')->exchangeArray( Helper_Breadcrumb::process($breadcrumbs) );
      
        // get form
        $form = self::$artistForm->artistImageCreate($artistName,$artist['artist_guid']);
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!$form->isValid($_POST)) {
                ...
                $image = Zend_Registry::get('configuration')->media->imagePath . '/' . $artist['artist_guid'] . '/' . $values['image']; 

                $data = array(
                    'guid'      => md5($values['image']),
                ...
                $imageId = self::$artistDb->createNewImage($data);

                $this->_redirect('picture/add/complete/id/'.$imageId);
            }
        } else {
            $this->view->form = $form;
        }
