 ...
 */
class Genre_RssController extends Zend_Controller_Action
{
    /* @access Public
     * @var object
     */
    public static $genreRss = null;
    ...
    {       
        ...
        if ( !$registry->get('moduleConfig')->module->genre->enabled ) {
            $this->_redirect('/error/module-not-enabled/module/Genra');
        }     

        if(!$registry->get('moduleConfig')->module->genre->rss) {
            $this->_redirect('/error/feature-not-enabled');
        }

        self::$genreRss = new Model_Genre_Rss;
    }
    ...
    public function newGenreAction()
    {
        ...
        $feedArray = array(
            'title' => 'New Genre',
            'link' => Zend_Registry::get('siteRootUrl'). '/genre/rss/new-genre',
            'description' => 'New Genre to VooDoo Music Box',
            'language' => 'en-us',
            'charset' => 'utf-8',
