 ...
 */
class Album_SoapController extends Zend_Controller_Action
{
    ...
    {       
        ...
        if(!$registry->get('moduleConfig')->module->album->api) {
    ...
    {        
        ...
        if(isset($_GET['wsdl']))
        {
            ...
            $wsdl->setClass('Model_Album_Soap');
            $wsdl->handle();
        } else {
            $server = new Zend_Soap_Server(Zend_Registry::get('siteRootUrl').'/album/soap?wsdl');
            $server->setClass('Model_Album_Soap');
            $server->handle();
        }      
    }
